...
from config.colors import (
    ...
    COLOR_PALETTE, HIGHLIGHT_1, HIGHLIGHT_2, HIGHLIGHT_3
)

# load directory
current_dir = os.path.dirname(os.path.abspath(__file__))

file_csv = os.path.join(current_dir, "results", "15-06-25", "NPV", "NPV_statistics.csv")
...
mean_cooling = df_only_cooling["mean_npv"].values
threshold_8_years_cooling = df_only_cooling["threshold_15_years"].values
...
plt.setp(markerline, marker='o', color=HIGHLIGHT_2, label="Dispersion for exceptional extremes (15 years)")
...
plt.title("Additional Benefit in the Worst-case - Dispersion from Mean for Each Scenario")
plt.ylim(0, 200)
plt.legend()
plt.show()
